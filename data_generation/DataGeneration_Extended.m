...
% Parameters for training and test datasets
...
h = kaiser(63,0.5);
imgSize = 112;
N_long = 1024*2; % Signal Length for generation
N = 1024;        % Final signal length
...
X_distortion_awgn_train = zeros(total_signals_train, 2, 1024);
X_distortion_echo_train = zeros(total_signals_train, 2, 1024);
X_distortion_cci_train = zeros(total_signals_train, 2, 1024);
...
X_distortion_awgn_test = zeros(total_signals_test, 2, 1024);
X_distortion_echo_test = zeros(total_signals_test, 2, 1024);
X_distortion_cci_test = zeros(total_signals_test, 2, 1024);
...
%                 waveformfolderCWD = fullfile(datasetCWD,waveform);
                for idx = 1:fps_train
                    ...
                    % Clean Signals. 
                    X_clean_signals_train(ik,1,:) = real(clean_signal);
                    X_clean_signals_train(ik,2,:) = imag(clean_signal);
        
                    % Distorted Signals. 
                    X_distorted_signals_train(ik,1,:) = real(distorted_signal);
                    X_distorted_signals_train(ik,2,:) = imag(distorted_signal);
        
                    
                    X_distortion_awgn_train(ik,1,:) = real(distortion_awgn);
                    X_distortion_awgn_train(ik,2,:) = imag(distortion_awgn);
        
                    X_distortion_echo_train(ik,1,:) = real(distortion_echo);
                    X_distortion_echo_train(ik,2,:) = imag(distortion_echo);
        
                    X_distortion_cci_train(ik,1,:) = real(distortion_cci);
                    X_distortion_cci_train(ik,2,:) = imag(distortion_cci);
 
                    Y_label_train(ik) = K; %class label
                    Y_SNR_train(ik) = desired_snr_db;
                    ik = ik+1;
...
%                 waveformfolderCWD = fullfile(datasetCWD,waveform);
                for idx = 1:fps_test
                    ...
                    % Clean Signals. 
                    X_clean_signals_test(ik,1,:) = real(clean_signal);
                    X_clean_signals_test(ik,2,:) = imag(clean_signal);
        
                    % Distorted Signals. 
                    X_distorted_signals_test(ik,1,:) = real(distorted_signal);
                    X_distorted_signals_test(ik,2,:) = imag(distorted_signal);
        
                    
                    X_distortion_awgn_test(ik,1,:) = real(distortion_awgn);
                    X_distortion_awgn_test(ik,2,:) = imag(distortion_awgn);
        
                    X_distortion_echo_test(ik,1,:) = real(distortion_echo);
                    X_distortion_echo_test(ik,2,:) = imag(distortion_echo);
        
                    X_distortion_cci_test(ik,1,:) = real(distortion_cci);
                    X_distortion_cci_test(ik,2,:) = imag(distortion_cci);
 
                    Y_label_test(ik) = K; %class label
                    Y_SNR_test(ik) = desired_snr_db;
                    ik = ik+1;
...
trainDirectory = 'Dataset_extended_train';
...
%% Save test dataset
testDirectory = 'Dataset_extended_test';
...
disp('Extended dataset generation complete!');
